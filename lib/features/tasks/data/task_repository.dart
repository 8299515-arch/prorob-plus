...
class ApiTaskRepository implements TaskRepository {
  ...
  Task _fromJson(Map<String, dynamic> json) {
    ...
    return Task(id: '${json['id'] ?? ''}', projectId: '${json['project_id'] ?? ''}', title: '${json['title'] ?? ''}', status: status, priority: priority, description: json['description'] as String?, dueDate: dueDate);
  }
}
