...
class _DocumentsPageState extends State<DocumentsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _error = 'Не удалось загрузить документы.';
    } finally {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Не удалось загрузить документ.')),
        );
      }
    } finally {
  ...
  Future<void> _delete(ProjectDocument document) async {
    try {
      ...
      if (mounted) {
        setState(
          () => _documents =
              _documents.where((item) => item.id != document.id).toList(),
        );
      }
    } catch (error, stackTrace) {
      AppLogger.instance.e(
              ...
              : _error != null
                  ...
                  : _documents.isEmpty
                      ? ListView(
                          children: const [
                            SizedBox(height: 180),
                            Center(child: Text('Документов пока нет.')),
                          ],
                        )
                      : ListView.separated(
